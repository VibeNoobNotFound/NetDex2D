# WindowsUpdateInstaller (updates/installers/WindowsUpdateInstaller.cs)

- Source: `scripts/updates/installers/WindowsUpdateInstaller.cs`
- Namespace: `NetDex.Updates.Installers`
- Purpose: WindowsUpdateInstaller in `updates/installers/WindowsUpdateInstaller.cs`. GitHub release updater state machine, checksum verification, and installers.
...
| `BuildScript(string zipPath, string appDir, string executableName)` | `private` | `string` |
| `EscapeBatchPath(string input)` | `private` | `string` |
...
- Source range: `scripts/updates/installers/WindowsUpdateInstaller.cs:19`
- Inputs:
...
- Source range: `scripts/updates/installers/WindowsUpdateInstaller.cs:83`
- Inputs:
  - `none`
- Output / side effects:
  - Reads/writes local filesystem or persistent config state.
...
- Source range: `scripts/updates/installers/WindowsUpdateInstaller.cs:97`
- Inputs:
...
- Source range: `scripts/updates/installers/WindowsUpdateInstaller.cs:135`
- Inputs:
  - `out string message`
- Output / side effects:
  - Reads/writes local filesystem or persistent config state.
...
- Source range: `scripts/updates/installers/WindowsUpdateInstaller.cs:158`
- Inputs:
...
### `BuildScript(string zipPath, string appDir, string executableName)`

- Signature: `private static string BuildScript(string zipPath, string appDir, string executableName)`
- Source range: `scripts/updates/installers/WindowsUpdateInstaller.cs:178`
- Inputs:
  - `string zipPath, string appDir, string executableName`
- Output / side effects:
  - Mutates local object state/fields.
- Validation rules:
  - No dedicated validation block; relies on surrounding call flow and type safety.
- Main call path (observed):
  - Calls `EscapeBatchPath(...)`
  - Calls `in(...)`
- Failure paths:
...
### `EscapeBatchPath(string input)`

- Signature: `private static string EscapeBatchPath(string input)`
- Source range: `scripts/updates/installers/WindowsUpdateInstaller.cs:207`
- Inputs:
  - `string input`
- Output / side effects:
